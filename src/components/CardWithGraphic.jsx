...
function CardWithGraphic({ title, description, imgUrl, alt, glowColor, rotatingBorderAngle, linkTo, setActiveLinkCallback }) {
  return (
    <>
      <div
        className="shadow-lg rotating-border-wrapper"
      ...
      >
        <Link to={linkTo} onClick={() => {
          ...
          <div className="h-full p-4 transition-all duration-500 ease-out bg-gray-200 border shadow-2xl rotating-border-card dark:bg-gray-700 rounded-2xl bg-white/10 dark:bg-gray-900/20 backdrop-blur-xl backdrop-saturate-150 border-white/20 dark:border-gray-700/30 shadow-black/10 dark:shadow-black/50 hover:border-white/40 dark:hover:border-gray-600/50">
